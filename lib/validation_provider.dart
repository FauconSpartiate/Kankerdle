...
class ValidationProvider extends StatefulWidget {
  ...
  const ValidationProvider(
      {super.key, required this.child, required this.database, required this.wordLen, required this.maxChances, required this.gameMode});
    ...
    var startNew = await showDialog<bool>(
        ...
        builder: (context) {
          return AlertDialog(
            ...
            content:
                Text(result ? "You won, my Kanker!" : "You Kanker, the answer was ${answer[0].toUpperCase() + answer.substring(1).toLowerCase()}!"),
            actions: [
              TextButton(
      ...
      onNotification: (noti) {
        if (noti.type == InputType.inputConfirmation) {
          if (curAttempt.length < widget.wordLen) {
            ...
            //Check validation
            if (ValidationProvider.validationDatabase.lookup(curAttempt) != null || curAttempt == "KANKER") {
              //Generate map
              ...
              for (int i = 0; i < widget.wordLen; i++) {
                if (curAttempt[i] == answer[i] || curAttempt == "Kanker") {
                  positionValRes[i] = 1;
